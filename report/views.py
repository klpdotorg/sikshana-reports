...
def main(request):
    if request.method == 'GET':
        topframe={'district':[],'block':[],'cluster':[],'school_id':[],'school_name':[],'clas':[],'total_students':[],'math_above_average':[],'math_average':[],'math_minimum':[],'math_below_minimum':[],'kannada_above_Average':[],'kannada_average':[],'kannada_minimum':[],'kannada_below_minimum':[],'year':[]}
        ...
        topframe['year']=[row.year for row in schools_master.objects.all().distinct('year')]
        ...
        if name_child not in ['District','Block','Cluster','School']:
            for row in table_child.objects.all().filter(name=name_child, clas=clas, year=year):
            ...
            for row in table_parent.objects.all().filter(name=name_parent, clas=clas, year=year):
